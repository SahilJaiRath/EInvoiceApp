...
namespace Einv_EwayBill_WebApp.Controllers
{
    public class EinvoiceController : Controller
    {
        ...
        [AllowAnonymous]       
        public IActionResult GenEinvoice (string ptype,string DocumentId,string dbunit,string CanCode)
        {
            ...
            string AppType = "";
            if (string.IsNullOrEmpty(ptype) || string.IsNullOrEmpty(DocumentId) || string.IsNullOrEmpty(dbname) || string.IsNullOrEmpty(Pwd) || string.IsNullOrEmpty(DbUser)|| string.IsNullOrEmpty(dbServer))
            {
            ...
            try
            {
                ...
                DataTable dtapptype = GetDataFromDB.GetDistinctData("APPTYPE", DocumentId);
                if(dtapptype.Rows.Count>0)
                {
                    ...
                    DataTable dt1 = GetDataFromDB.GetDistinctData("GETID", DocumentId);
                    if(dt1.Rows.Count>0)
                    {
                        dtinvewb = dt1;
                    }
                    ...
                    {
                        dtinvewb = GetDataFromDB.GetDistinctData("CHECKINVEWB", DocumentId);
                    }
                ...
                {               
                    if (AppType.ToUpper() == "EI")
                    {
                        ...
                        {
                            dtInvL = GetDataFromDB.GetDistinctData("GINV", DocumentId);
                        ...
                        {
                            dtInvL = GetDataFromDB.GetDistinctData("CINV", DocumentId);
                            if (dtInvL.Rows.Count > 0)
                            {
                                var access_token = Taxilla_EinvoiceAPI.AccessToken(dtInvL).Result;
                                var value2 = JsonConvert.DeserializeObject<AccessTokenResult>(access_token);
                        ...
                        {
                            dtInvL = GetDataFromDB.GetDistinctData("GETINVBYIRN", DocumentId);
                            if (dtInvL.Rows.Count > 0)
                            {
                                string finalString = ClsDynamic.GenerateUniqueNumber();
                                var access_token = Taxilla_EinvoiceAPI.AccessToken(dtInvL).Result;
                        ...
                        {
                            dtInvL = GetDataFromDB.GetDistinctData("EWBBYIRN", DocumentId);
                            if (dtInvL.Rows.Count > 0)
                            {
                                var access_token = Taxilla_EinvoiceAPI.AccessToken(dtInvL).Result;
                                var value2 = JsonConvert.DeserializeObject<AccessTokenResult>(access_token);
                        ...
                        {
                            dtInvL = GetDataFromDB.GetDistinctData("CEWBBYIRN", DocumentId);
                            if (dtInvL.Rows.Count > 0)
                            {
                                var access_token = Taxilla_EinvoiceAPI.AccessToken(dtInvL).Result;
                                var value2 = JsonConvert.DeserializeObject<AccessTokenResult>(access_token);
                        ...
                        {
                            dtInvL = GetDataFromDB.GetDistinctData("EXTQR", DocumentId);
                            if (dtInvL.Rows.Count > 0)
                            {
                                var access_token = Taxilla_EinvoiceAPI.AccessToken(dtInvL).Result;
                                var value2 = JsonConvert.DeserializeObject<AccessTokenResult>(access_token);
                        ...
                        {
                            dtInvL = GetDataFromDB.GetDistinctData("EXTINV", DocumentId);
                            if (dtInvL.Rows.Count > 0)
                            {
                                var access_token = Taxilla_EinvoiceAPI.AccessToken(dtInvL).Result;
                                var value2 = JsonConvert.DeserializeObject<AccessTokenResult>(access_token);
                        ...
                        {
                            dtInvL = GetDataFromDB.GetDistinctData("GQRIMG", DocumentId);
                            if (dtInvL.Rows.Count > 0)
                            {
                                var access_token = Taxilla_EinvoiceAPI.AccessToken(dtInvL).Result;
                                var value2 = JsonConvert.DeserializeObject<AccessTokenResult>(access_token);
                        ...
                        {
                            dtInvL = GetDataFromDB.GetDistinctData("GetQRIMG", DocumentId);
                            if (dtInvL.Rows.Count > 0)
                            {
                                var access_token = Taxilla_EinvoiceAPI.AccessToken(dtInvL).Result;
                                var value2 = JsonConvert.DeserializeObject<AccessTokenResult>(access_token);
                ...
                else
                {
                    if(AppType=="EI")
                    {
                        ...
                        {
                            dtInvL = GetDataFromDB.GetDistinctData("DTLINV", DocumentId);
                            if (dtInvL.Rows.Count > 0)
                            {
                                dynamic access_token = GetAccessToken.Einvoice_API_Login(dtInvL).Result;
                                if (access_token != "")
